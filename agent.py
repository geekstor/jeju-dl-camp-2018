...
class C51Agent():
    class Model():
        def __init__(self, session, num_actions, train_net):
            ...
            with tf.variable_scope("common"):
                ...
                self.post_mul = tf.multiply(tf.range(start=-10, limit=10,
                                               delta=(20/51)), self.q_dist)
            ...
            if not train_net:
                ...
                self.argmax_action = tf.argmax(self.actions, axis=1, output_type=tf.int32)
                ...
                self.argmax_action_distribution = tf.gather_nd(self.q_dist,
                                                               tf.stack(
                                                                   (tf.range(start=0, limit=tf.shape(self.actions)[0]),
                                                                    self.argmax_action), 1))
                ...
                self.Tz = tf.clip_by_value(tf.reshape(self.r, [-1, 1]) + 0.99 * tf.multiply(
                    tf.cast(tf.reshape(self.t, [-1, 1]), tf.float32), tf.range(start=-10, limit=10, delta=(20/51))),
                                 clip_value_min=-10, clip_value_max=10)

                # Compute bin number (will be floating point).
                self.b = (self.Tz - (-10))/(20/51)
                ...
                self.m_l = tf.matmul(tf.one_hot(indices=tf.cast(self.l, tf.int32), depth=51),
                          tf.reshape(self.argmax_action_distribution * (self.u - self.b), [-1, 1, 51]),
                                     transpose_b=True)

                # Add weight to the lower bin based on distance from upper bin to
                # approximate bin index b.
                self.m_u = tf.matmul(tf.one_hot(indices=tf.cast(self.u, dtype=tf.int32), depth=51),
                          tf.reshape(self.argmax_action_distribution * (self.b - self.l), [-1, 1, 51]),
                                     transpose_b=True)
                ...
                self.weighted_m = tf.clip_by_value(self.m *
                                 tf.range(start=-10, limit=10, delta=(20/51)),
                                 clip_value_min=-10, clip_value_max=10)

                self.targn_summary = tf.summary.histogram("target_distribution",
                                        self.weighted_m)
            else:
                ...
                self.action_q_dist = tf.gather_nd(self.q_dist,
                                                  tf.stack((tf.range(start=0, limit=self.action_placeholder.shape[0]),
                ...
                self.trnn_summary = tf.summary.merge([tnd_summary, l_summary, grad_norm_summary])
    ...
    def update(self, x, a, r, x_p, t):
        ...
        for _ in range(1):
            ...
            #
            # else:
            targn_summary, m = self.sess.run([self.target_net.targn_summary, self.target_net.m], feed_dict={self.target_net.x: batch_x_p,
        ...
        if self.steps > 0 and self.steps % 5000 == 0:
