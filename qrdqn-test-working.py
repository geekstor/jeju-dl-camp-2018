...
class QRDQN:
    def __init__(self, num_inputs, num_actions, num_quantiles):
        ...
        # theta_i = tf.tile(tf.expand_dims(self.dist_of_chosen_actions, -1),
        #                   [1, 1, self.num_quantiles])
        # T_theta_j = tf.tile(tf.expand_dims(self.target, -2),
        #                     [1, self.num_quantiles, 1])
        # tau_i = tf.tile(tf.expand_dims(self.quantile_midpoints, -1), [1, 1, self.num_quantiles])
        #
        # self.huber_loss_interval = 1 # kappa
        # # Huber loss of T(theta_j) - theta_i
        # abs_error = tf.abs(T_theta_j - theta_i)
        # quadratic = tf.minimum(abs_error, self.huber_loss_interval)
        # huber_loss = self.huber_loss_interval * (abs_error - quadratic) + \
        #              0.5 * quadratic ** 2
        #
        # # Quantile Huber loss
        # quantile_huber_loss = tf.abs(tau_i - tf.cast(abs_error < 0, dtype=tf.float32)) * \
        #                       huber_loss
        #
        # # Quantile regression loss (the probability for each quantile is 1/num_quantiles)
        # quantile_regression_loss = tf.reduce_sum(quantile_huber_loss) / float(self.num_quantiles)
...
# epsilon_by_frame = lambda frame_idx: 0.01 + (1 - 0.01) * \
#                                      (2e4 - min(frame_idx, 2e4)) / 2e4
